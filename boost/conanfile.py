...
class BoostConan(ConanFile):
    ...
    default_options.append("without_context=True")
    # default_options.append("without_coroutine=True")
    # default_options.append("without_locale=True")
    default_options.append("without_test=True")
    ...
    exports_sources = ["%s.tar.gz" % folder_name]
    ...
    def configure(self):
        if self.zip_bzip2_requires_needed:
            self.requires("bzip2/1.0.6@seewolive/testing")
            self.options["bzip2"].shared = False

            self.requires("zlib/1.2.11@seewolive/testing")
    ...
    def build(self):
        ...
        # JOIN ALL FLAGS
        b2_flags = " ".join(flags)
        full_command = "%s %s -j%s --abbreviate-paths -d2 -q" % (b2_exe, b2_flags, tools.cpu_count())
        #full_command = "%s %s %s --abbreviate-paths -d2 -q" % (b2_exe, b2_flags, tools.cpu_count())
